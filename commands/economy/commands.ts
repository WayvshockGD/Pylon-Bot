...
commands.on(
    ...
    async (message, { command }) => {
        if (!command) {
            ...
            embed.addField({
                name: 'Aliases',
                value: `${cmd.executor.options.aliases
...
const nextDrawText = () => {
    ...
    return `next draw is in ${minutes} ${minutes === 1 ? 'minute' : 'minutes'
...
const setDefaultReply = (commandGroup: discord.command.CommandGroup) => {
    commandGroup.default(
        ...
        async (message) =>
            await message.reply(
                `${discord.decor.Emojis.NO_ENTRY} unknown potato command, try \`${cmd.commandPrefixes[0]}potato help\``
            )
    );
};
...
discord.on(discord.Event.MESSAGE_CREATE, async (message: discord.Message) => {
    ...
    if (await potatoKV.get<boolean>('cooldown')) {
        if (message.content === discord.decor.Emojis.POTATO) {
            ...
            await message.reply(
                new discord.Embed({
                    title: `${poisonous ? discord.decor.Emojis.SKULL : discord.decor.Emojis.POTATO
                        } potato claimed ${discord.decor.Emojis.POTATO}`,
                    description: `${message.author.getTag()} ${poisonous
    ...
    (subcmd) => {
            ...
            async (message, { }) => {
                ...
                await message.reply(
                    new discord.Embed({
                        ...
                        description: `${message.author.getTag()} has ${currentCount} potato${currentCount === 1 ? '' : 's'
        ...
        subcmd.on(
            ...
            async (message, { who }) => {
                ...
                await message.reply(
                    new discord.Embed({
                        title: `${discord.decor.Emojis.POTATO} potato count ${discord.decor.Emojis.POTATO}`,
                        description: `${who.getTag()} has ${currentCount} potato${currentCount === 1 ? '' : 's'
                            }. ${discord.decor.Emojis.POTATO.repeat(
                                Math.min(currentCount, 100)
                            )}`,
                        color: 0x11111c,
            ...
            async (message, { count }) => {
                ...
                await message.reply(
                    new discord.Embed({
                        ...
                        description: `Your gambling ${won ? 'paid off' : 'sucked'}, you ${won ? 'won' : 'lost'
                            } ${count} potato${count === 1 ? '' : 's'}, ${won ? 'giving you' : 'leaving you with'
                            } a total of ${newCount} potato${newCount === 1 ? '' : 's'
                            }. ${discord.decor.Emojis.POTATO.repeat(newCount)} ${won
            ...
            async (message, { who, count }) => {
                ...
                await message.reply(
                    new discord.Embed({
                        ...
                        description: `Your thievery ${success ? 'paid off' : 'sucked'
                            }, you ${success ? 'stole' : 'gave'} ${count} potato${count === 1 ? '' : 's'
                            } ${success ? 'from' : 'to'} ${who.getTag()}, ${success ? 'giving you a total of' : 'leaving you with'
                            } ${newUserPotatos} potato${newUserPotatos === 1 ? '' : 's'
                            }. ${discord.decor.Emojis.POTATO.repeat(newUserPotatos)} ${success
                                ? discord.decor.Emojis.CHART_WITH_UPWARDS_TREND
                                : discord.decor.Emojis.CHART_WITH_DOWNWARDS_TREND
                            }`,
                        color: 0x11111c
        ...
        subcmd.on(
            ...
            async (message, { who, count }) => {
                ...
                await message.reply(
                    `you gave ${count} potato${count === 1 ? '' : 's'
            ...
            async (message, { count }) => {
                ...
                for (const entry of userMap.slice(0, Math.max(3, count - 1))) {
                    ...
                    description += `\` ${MEDALS[place] ||
                        `${(place + 1).toString().padStart(2, ' ')} `} \` **${user?.username
                ...
                if (ownIndex >= count) {
                    ...
                    description += `\` ${(ownIndex + 1)
                        .toString()
                        .padStart(2, ' ')} \` **${message.author.username}**#${message.author.discriminator
                        } - ${sorted[ownIndex].value} potato${sorted[ownIndex].value === 1 ? '' : 's'
                        }`;
                } else if (count > 3) {
                    const { user, potatos } = userMap[count - 1];
                    description += `\` ${count.toString().padStart(2, ' ')}  \` **${user?.username
                        }**#${user?.discriminator} - ${potatos.toLocaleString()} potatos\n`;
                }
        ...
        subcmd.on(
            ...
            async (message) => {
                ...
                if (lastPotato)
                    return await message.reply(
                        `there is already an active potato waiting to be picked up in <#${lastPotato.split('-')[0]
                        }>!`
                    );
        ...
        const lottery = subcmd.subcommandGroup({
            name: 'lottery',
            ...
            async (message) => {
                ...
                await message.reply(
                    `${Object.keys(lotteryData).length
                        ...
                        lotteryData
                    ).reduce((a, b) => a + b, 0)} potatos${lotteryData[message.author.id]
                        ? `. you are in the pool with ${lotteryData[message.author.id]} ${lotteryData[message.author.id] === 1 ? 'potato' : 'potatos'
                        }`
                        : ''
                    }. ${nextDrawText()}`
                );
        ...
        lottery.on(
            {
                name: 'deposit',
                description: 'deposits potatos into the lottery pool'
            },
            (args) => ({ count: args.integer() }),
            async (message, { count }) => {
                const channel = await discord.getGuildTextChannel(
                ...
                await message.reply(
                    `you deposited ${count} ${count === 1 ? 'potato' : 'potatos'
                    }, there are ${totalCount} ${totalCount === 1 ? 'potato' : 'potatos'
                    } from ${gamblerCount} ${gamblerCount === 1 ? 'gambler' : 'gamblers'
                    } in the pool${lotteryData![message.author.id] !== count
        ...
        const shop = subcmd.subcommandGroup({
            name: 'shop',
        ...
        shop.on(
            ...
            async (message) => {
                ...
                await message.reply(
                    new discord.Embed({
                        title: 'Potato Shop',
                        description: `**Available Items**\nuse \`${cmd.commandPrefixes[0]}potato shop buy <item>\` to purchase an item listed here`,
                        fields
                    })
                );
            }
        ...
        shop.on(
            ...
            async (message, { item }) => {
                ...
                if (!itemObj || !itemObj.enabled)
                    return await message.reply(
                        `invalid potato item. use \`${cmd.commandPrefixes[0]}potato shop list\` to get a list of all available items`
                    );
                ...
                if (purchase)
                    return message.reply(
                        `You already bought this item!${purchase.expiresAt
...
pylon.tasks.cron(
    ...
    async () => {
        ...
        await channel.sendMessage(
            ...
            ) / 10}% chance)! they won ${idList.length
